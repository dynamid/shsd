...
@app.route('/index')
def index():
    # geojson="api/getGeoJSON" #url_for('getGeoJSON'),
    return render_template('index.html', user=getCurrentUser(), center_map=getAvgPositions(getCurrentUser()), colors_to_print=getColorsFromDB(getCurrentUser()))
...
def addDevice(ip, hw):
    ...
    try:
        ...
        Session.execute(devices.update().where(
            devices.c.hw == hw).values(lastseen=now))
    Session.commit()
    Session.remove()
...
def addConnectionJSON():
    ...
    for connection in connections:
        date = datetime.date(int(connection['year']), int(
            connection['month']), int(connection['day']))
        ip = connection['ip']
        ...
        if (known == 0):
            if (geoloc == 'onyphe' or not isipv4(ip)):
                Session.execute(accounts.insert(), [
                    {'login': user, 'ip': ip, 'firstseen': date,
                        'lastseen': date, 'is_populated': False}
                ])
                ...
                if (new_as is not None and new_geoloc is not None):
                    ...
                    print("no geoip entry for " + ip +
                          ", failing back to onyphe")
                    Session.execute(accounts.insert(), [
                        {'login': user, 'ip': ip, 'firstseen': date,
                            'lastseen': date, 'is_populated': False}
                    ])
                ...
                Session.execute(accounts.insert(), [
                    {'login': user, 'ip': ip, 'firstseen': date,
                        'lastseen': date, 'is_populated': True}
                ])
        else:
            s = select([accounts.c.login, accounts.c.ip, accounts.c.lastseen]).where(and_(
            ...
            for row in Session.execute(s):
                ...
                if (date > olddate):
                    Session.execute(accounts.update().where(
                        and_(accounts.c.ip == ip, accounts.c.login == user)).values(lastseen=date))
    try:
        Session.commit()
    except:
        Session.rollback()
...
def colorAs(user):
    c = select([ascolors.c.id_color]).where(
        ascolors.c.uid == user).distinct().count()
...
def getASColor(asn, user):  # return the color and creates it if needed
    ...
    # Si le couple as/user existe, on récupère la couleur
    nb_color = Session.execute(nb_c).scalar()

    if nb_color != 0:
        c = select([ascolors.c.id_color]).where(
            and_(ascolors.c.uid == user, ascolors.c.ip_as == asn))
        # il n'y a qu'un seul résultat (normalement)
        for rows in Session.execute(c):
...
def getColorsFromDB(user):
    c = select([ascolors.c.id_color, ascolors.c.ip_as]
               ).where(ascolors.c.uid == user)
    col = []
    for row in Session.execute(c):
        s = select([accounts.c.ip_org]).where(and_(accounts.c.login ==
                                                   user, accounts.c.ip_as == row[ascolors.c.ip_as])).distinct()
...
def markerSize(first, last):
    delta = last - first
    size = "small"
    # s'il y a moins de 5 jours d'écart entre la première et la dernière connexion
    if delta.days < 6 and last.month == first.month:
        size = "large"
...
# point central
    ...
    max = select([func.max(accounts.c.ip_latitude), func.max(
        accounts.c.ip_longitude)]).where(accounts.c.login == user).distinct()
